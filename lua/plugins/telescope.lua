            ...
            defaults = {
                ...
                mappings = {
                    i = {                                            -- 插入模式
                        ...
                        ['<C-c>'] = function(prompt_bufnr)           -- 关闭
                            local ok = pcall(actions.close, prompt_bufnr)
                            if not ok then
                                vim.notify("不支持此操作", vim.log.levels.WARN)
                            else
                                vim.cmd("stopinsert") -- 关闭后强制回正常模式
                            end
                        end,
                        ['<CR>'] = function(prompt_bufnr)  -- 选中
                            local ok = pcall(actions.select_default, prompt_bufnr)
                            if not ok then
                                vim.notify("不支持此操作", vim.log.levels.WARN)
                            else
                                vim.cmd("stopinsert") -- 关闭后强制回正常模式
                        ...
                        ['<c-CR>'] = function(prompt_bufnr)  -- 选中
                            local ok = pcall(actions.select_default, prompt_bufnr) if not ok then
                                vim.notify("不支持此操作", vim.log.levels.WARN)
                            else
                                vim.cmd("stopinsert") -- 关闭后强制回正常模式
                            end
                        end,
                        ['<c-o>'] = function(prompt_bufnr)  -- 选中
                            local ok = pcall(actions.select_default, prompt_bufnr)
                            if not ok then
                                vim.notify("不支持此操作", vim.log.levels.WARN)
                            else
                        ...
                        ['<C-x>'] = function(prompt_bufnr)   -- 水平分屏打开
                        ...
                        ['<C-t>'] = function(prompt_bufnr) -- 新标签页打开
