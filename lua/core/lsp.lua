...
vim.api.nvim_create_autocmd("LspAttach", {
    ...
    callback = function(event)
        ...
        -- 查看引用
        map("<C-S-d>", "<cmd>Lspsaga finder def+ref+imp<CR>", "Go to definition and refinition (Lspsaga)")
        -- 查看定义
        map("<C-S-i>", "<cmd>Lspsaga peek_definition<CR>", "Peek definition (Lspsaga)")
        -- 跳转定义
        map("<C-S-j>", "<cmd>Lspsaga goto_definition<CR>", "Goto definition (Lspsaga)")
        -- 跳转到上次跳转的地方
        map("<C-S-k>", "<C-t>", "Go back to previous position")
        -- 查看声明
        map("<leader>ii", "<cmd>Lspsaga hover_doc<CR>", "Hover documentation (Lspsaga)")
        -- 批量重命名
        map("<leader>rn", "<cmd>Lspsaga rename ++project<CR>", "Smart rename (Lspsaga)")
        -- 显示行诊断
        map("<leader>ll", "<cmd>Lspsaga show_line_diagnostics<CR>", "Show line diagnostics (Lspsaga)")
        -- 显示所有诊断
        map("<leader>la", "<cmd>Telescope diagnostics<CR>", "Show all diagnostics")
        -- 查看代码建议
        map("<leader>ca", "<cmd>Lspsaga code_action<CR>", "Code actions (Lspsaga)")
        -- 查看上一个诊断
        map("<leader>lj", "<cmd>Lspsaga diagnostic_jump_prev<CR>", "Jump to previous diagnostic")
        -- 查看下一个诊断
        map("<leader>lk", "<cmd>Lspsaga diagnostic_jump_next<CR>", "Jump to next diagnostic")
        -- 使用lsp默认的format程序（normal和visual模式）
        map("==", "<cmd>lua vim.lsp.buf.format()<CR>", "Format buffer", { "n", "v" })
        -- 添加源文件/头文件转换
        map("<C-o>", "<cmd>lua vim.lsp.buf_request(0, 'textDocument/switchSourceHeader', { uri = vim.uri_from_bufnr(0) })<CR>", "Switch source/header")
        -- 删除lsp默认快捷键K
...
local function lsp_info()
    ...
    print("󰈔 检测到的文件类型: " .. vim.bo.filetype)        -- 当前文件类型
    print("󰈮 缓冲区ID: " .. bufnr)                         -- 当前缓冲区ID
    print("󰈔 根目录: " .. (vim.fn.getcwd() or "N/A"))      -- 当前工作目录
    print("")
    ...
    for i, client in ipairs(clients) do
        ...
        print("  根目录: " .. (client.config.root_dir or "未设置"))  -- 项目根目录
